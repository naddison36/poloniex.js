module.exports = (function() {
    ...
    Poloniex.prototype = {
        ...
        _request: function(options, callback)
        {
            ...
            var requestDesc = util.format('%s request to url %s with strict ssl %s, query string %s and params %s',
                options.method, options.url, Poloniex.STRICT_SSL, JSON.stringify(options.qs), JSON.stringify(options.form));

            request(options, function(err, response, body)
            {
                if (err)
                {
                    return callback(new VError(err, "Failed %s", requestDesc), null);
                }
                else if ((typeof body === 'undefined' || body === null))
                {
                    return callback(new VError('Empty response from %s', requestDesc), null);
                }
                else if (body.error)
                {
                    return callback(new VError('Error returned from %: %s', requestDesc, body.error), null);
                }
